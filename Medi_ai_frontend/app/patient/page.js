...
function HealthDataCard() {
  ...
  // Build chart data from biometrics grouped by date, mixed with dummy data for UI aesthetics
  ...
  last7Days.forEach((d, i) => {
    ...
    dayMap[dateKey] = { name: label, _ts: d.getTime(), bp: bpVals[i], hr: hrVals[i], temp: tempVals[i] };
  });
